...
Wenn sich die grundlegenden, sich ständig wiederholenden Aufgaben der täglichen Arbeit auch nur um ein wenig effizienter erledigen lassen, hat das eine grosse Auswirkung auf die Produktivität.
Deshalb bringt es grosse Vorteile, sich mit Shell Commands oder Editing Shortcuts zu befassen. Man muss nur ein wenig Zeit investieren.\\
...
Abgesehen von der offensichtlichen Funktionalität wie Branching oder der einfache Rollback kann vor allem auch das automatisierte Builden, Testen oder Deployen von Code mittels 
Version Control Systemen erfolgt werden.

\subsection{Debugging}
Zum diesem Thema geben die Authoren verschiedene gute Tipps
\begin{itemize}[itemsep=4pt]
    \item Fix the Problem, not the Blame \\ Wichtiger ist, dass das Problem behoben wird, nicht wer schuld ist.
    \item Don't Panic \\ Das Problem soll systematisch und überlegt angegangen werden, so wie auch die tägliche Arbeit erledigt wird. Im Panik-Modus werden wahrscheinlich nicht 
    die besten Entscheidungen getroffen. 
    \item Debugging Strategien \\ Abgesehen von den verschiedenen Vorgehensweisen beim Debugging (\emph{Rubber Duck Debugging, Binary Chop, Elimination, Logging}) 
...
\emph{Listen to your lizard brain: } Auf unsere Instinkte zu hören wird einem als Mensch unweigerlich abtrainiert. Das Unterbewusstsein als solches 
kann aber sehr oft hilfreich sein, und kann auch mittels verschiedenen Tricks wieder trainiert werden.  

\emph{Programming by Coincidence: } Sich seiner Umgebung bewusst sein, hilft dem Frosch im heissen Wasser (siehe Boiled Frogs). Dasselbe gilt für seinen Code,
und die Funktionsweise davon. Etwas, das zwar funktioniert aber nicht wirklich verstanden wird, kann im nächsten Moment zu Problemen führen, mit denen man nicht rechnet.
...
\emph{Design by Contract:} Methoden stellen \emph{Preconditions} an den Aufrufer. Werden diese eingehalten, 
    garantieren Sie gewisse Anforderungen an das Resultat, die \emph{Postconditions}. \emph{Class Invariants} sind Garantien, 
...
\emph{Assertive Programming:} Assertions können verwendet werden, um sicherzustellen dass das Unmögliche nicht eintritt. Fälle, die aus Sicht des Programmierers "garantiert nie" auftreten, 
...
\emph{Test to code:} Sich Gedanken darüber machen, wie eine Funktionalität getestet werden soll, kann in vielen Fällen dazu beitragen, dass die effektive Umsetzung anschliessend
einfacher und besser umgesetzt wird. Man betrachtet den Code nicht als Autor, sondern als Nutzer. Das Ziel ist klarer, die Umsezung kann Schritt für Schritt erfolgen und wird direkt getestet. 
...
Refactoring, das zu besserem, einfacherem und klarerem Code beiträgt, sollte wann immer möglich ausgeführt werden. 
...
\emph{Stay safe out there:} Heutzutage kann nicht mehr davon ausgegangen werden, dass sich niemand für eine Applikation interessiert, wenn sie nicht bekannt ist. 
...
in der Software weiterverwendet werden. Kryptographie auf keinen Fall selber implementieren.
...
\emph{Globale Daten vermeiden:} Globale Daten verbinden alle Module miteinander, die darauf zugreifen. 
...
\subsection{Events}
In der realen Welt geschehen Dinge, und wir reagieren darauf. Software sollte, um uns bestmöglich zu unterstützen, dasselbe können. 
Das Prinzip von Events kann auf verschiedene Arten implementiert werden, eine sehr elegante kann das Design einer \emph{Finite State Machine} sein. 
Diese definiert verschiedene Zustände, Ereignisse und dazugehörige Übergänge. Zwei weitere Varianten sind \emph{Publish/Subscribe} oder \emph{Streams.} 
...
\emph{Shared State is Incorrect State}

\emph{Actors and Processes}
...
Eine gute Schätzung für alles ausser ganz simplen Anforderungen beinhaltet deshalb stets mehrere Szenarien.

\subsection{Requirements}
Der Grundsatz: \emph{Niemand weiss genau, was er will.}
Der Kunde weiss meist auch nicht, was die Software alles bieten kann. Als Softwareentwickler gehört es zur Aufgabe, mit dem Kunden zusammen 
herauszufinden, möglicherweise über zahlreiche Iterationen, ob das Produkt wirklich den Anforderungen entspricht, bzw. 
wie diese Anforderungen denn überhaupt aussehen. Unklarheiten müssen angesprochen und bereinigt werden. 

Idealerweise kann der Entwickler sich mit den späteren End-Usern direkt austauschen, um deren Arbeitsweise und Bedürfnisse zu erkennen.
...
Oft hilft eine Pause, Ablenkung, das Problem einer anderen Person erklären.
...
andere Personen verbessert das Resultat, und mögliche Probleme, die alleine viel Zeit in Anspruch nehmen würden, können in der Gruppe meist schneller gelöst werden.
...
In Stress-Situationen: DON'T PANIC AND BRING A TOWEL.\\
